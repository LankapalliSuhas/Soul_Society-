...
export default function OccupancyCounter({ occupancy, trend }) {
  return (
    <motion.div
      ...
      className="p-8 border border-white/10 bg-[#111111]/40 backdrop-blur-sm rounded-lg flex flex-col justify-between h-full"
    >
      <h3 className="text-white/60 text-xs tracking-widest uppercase mb-4">Current Occupancy</h3>
      <div className="flex items-end justify-between">
        <div className="text-7xl font-light tracking-tighter text-white/90">
          {occupancy ?? '--'}
        </div>
        <div className={`text-sm mb-2 ${trend > 0 ? 'text-[#E11D48]' : 'text-white/40'}`}>
